...
df, pca_df, pca = PCA_outlier_remove(df, 0.05)

df = normilize(df, df.columns[1:])
test_df = normilize(test_df, test_df.columns[1:])

df.to_csv('../../data/preprocessed/wine/train_wine.csv', index=False)
test_df.to_csv('../../data/preprocessed/wine/test_wine.csv', index=False)
