...
# read data
df = pd.read_csv("../../data/statlog/statlog.csv")
...
df, pca_df, pca = PCA_outlier_remove(df, 0.25)

df = normilize(df, df.columns[:-1])
test_df = normilize(test_df, test_df.columns[:-1])

df.to_csv('../../data/preprocessed/statlog/train_statlog.csv', index=False)
test_df.to_csv('../../data/preprocessed/statlog/test_statlog.csv', index=False)
